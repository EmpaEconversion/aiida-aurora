...
def measure(time, record_every_dt):
    ...
    times = np.arange(0., time + 0.0001, record_every_dt)
    data = {
        'time': times.tolist(),
...
def execute_experiment(battery_d, experiment_d):
    ...
    # perform measurement
    output_data = measure(experiment_d['parameters']['time'], experiment_d['parameters']['record_every_dt'])
...
def main():
    ...
    parser.add_argument('experiment_specs', type=str, help='A JSON file with the experiment specifications.')
    parser.add_argument('-o', '--output', type=str, default='exp.out', help='Name of the output file.')
    args = parser.parse_args()

    try:
        battery_specs = json.load(open(args.battery_specs, 'r'))
        experiment_specs = json.load(open(args.experiment_specs, 'r'))
